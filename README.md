...
8. running 中 Worker 崩溃后的恢复仍依赖心跳/租约续期，是已知限制。
...
- claimed 超时回收；running 崩溃恢复为已知限制。
- Worker 认领后未进入 running 会主动 release。

### 状态机 / 原子上报
- 只有持有者能推进；终态不能乱跳。
- 整任务提交必须覆盖全部 Step；全 failure 但均已 success 时不做 done（保持原状态并返回 `ignored_failures`）。
...
- 已 done/failed 任务不能再上报。
- API 无认证、异常返回内部信息、看板无分页、Step 为 mock、无连接池/自动归档（生产化已知限制）。
